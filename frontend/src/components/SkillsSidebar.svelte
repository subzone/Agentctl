...
<aside class="tside">
  <div class="thead">
    ...
    <button class="mini" title="New skill" on:click={() => dispatch('new')}>＋</button>
  </div>
    ...
    {#each skills as s}
      <button class="titem" class:active={activeName === s.name} on:click={() => dispatch('select', s)}>
        ...
        <div class="tinfo">
          ...
          <div class="tdesc">{s.description || 'reusable instructions'}</div>
        </div>
      </button>
    {/each}
    {#if skills.length === 0}
      <div class="tempty">No skills yet.<br />Create one to guide the agent on every task.</div>
    {/if}
  </div>
