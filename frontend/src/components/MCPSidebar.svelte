...
<aside class="tside">
  <div class="thead">
    ...
    <button class="mini" title="New MCP server" on:click={() => dispatch('new')}>＋</button>
  </div>

  <div class="tlist">
    {#each servers as s}
      <button class="titem" class:active={activeName === s.name} on:click={() => dispatch('select', s)}>
        <span class="ticon">🔌</span>
        <div class="tinfo">
          <div class="tname">
            {s.name}
            {#if s.error}<span class="terr" title={s.error}>!</span>{/if}
          </div>
          <div class="tdesc">{s.description || s.transport || 'mcp server'}</div>
        </div>
      </button>
    {/each}
    {#if servers.length === 0}
      <div class="tempty">No MCP servers yet.<br />Add one to give the agent external tools.</div>
    {/if}
  </div>
